...
function renderMeetingReport() {
  ...
  const { previousDate, start, end } = getMeetingWindow(date);
  const windowText = `${formatDateTime(start)} - ${formatDateTime(reportEntry?.window?.to || end)}`;
  setText('#meetingWindowText', `自动获取 ${previousDate} 18:00 到当前的提交、Review 和任务领取信号。`);
