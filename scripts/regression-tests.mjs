...
await test('mobile app shell exposes bottom navigation and safe touch layout', async () => {
  ...
  // 侧边栏移动端：汉堡按钮 + overlay
  assert.match(html, /id="sbMobileToggle"/);
  assert.match(html, /id="sbOverlay"/);
  assert.match(html, /class="sidebar"/);
  assert.match(app, /aria-expanded/);

  // CSS: 底部导航 safe area + touch targets
  ...
  // 侧边栏移动端 CSS：slide-in + overlay
  assert.match(css, /\.sidebar\s*\{[\s\S]*position:\s*fixed/);
  assert.match(css, /\.sb-overlay\s*\{[\s\S]*position:\s*fixed/);
});
